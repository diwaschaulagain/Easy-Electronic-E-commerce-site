    ...
    public class OrderController : Controller
    {
        ...
        public ActionResult ViewOrder(int id)
        {
            using (EasyElecDBEntities db = new EasyElecDBEntities())
            {
                ...
                {
                    lstod.Add(new OrderDetailViewModel() { ProductId = Convert.ToInt32(item.ProductId), ProductName = item.tblProduct.ProductName, Quantity = Convert.ToInt32(item.Quantity), UnitPrice = Convert.ToDecimal(item.UnitPrice) });
                }
